...
public class IssueExtractor {
	...
	public List<IssueInfo> extract(String issueFolderPath,String projectName){
		...
		for(File oneIssueFolder: issuesFolder.listFiles()){//An issue folder
			...
			for(File issueFileOrPatchesFolder: oneIssueFolder.listFiles()){				
				...
				if(fileName.endsWith(".json")){//issue file
					...
					IssueInfo issueInfo = handleWithAnIssue(projectName,issueFileOrPatchesFolder);
	...
	private IssueInfo handleWithAnIssue(String projectName,File issueFile) {
...
//				System.out.printf("outwardIssue: name:%s,linkIssueId:%s\n",name,linkIssueId);
		...
		issueInfo.setProjectName(projectName);
	...
	public static void main(String[] args) {
		...
		extractor.extract("data/issues/lucene", "lucene");
//		extractor.extract("data/issues/lucene2", "lucene");
		
//		extractor.handleWithAnIssue("lucene",new File("data/issues/lucene/12718530/LUCENE-5735.json"));
//		extractor.handleWithAnIssue("lucene",new File("data/issues/lucene/12333383/LUCENE-565.json"));
//		extractor.handleWithAnIssue("lucene",new File("C:/Users/shutear/Downloads/LUCENE-5735.json"));
		
//		Map<String,IssueUserInfo> userMap = extractor.getUserMap();
