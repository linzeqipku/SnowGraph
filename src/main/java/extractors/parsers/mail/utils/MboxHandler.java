...
import extractors.parsers.mail.entity.MailInfo;
...
public class MboxHandler extends AbstractContentHandler {
    ...
    private void createUserNode(Node mailNode, String userName, String userAddress, boolean sender) {
        ...
        if (!mailUserMap.containsKey(userAddress)) {
            ...
            userNode.setProperty(MailListKnowledgeExtractor.MAILUSER_MAIL, userAddress);
        ...
        if (sender)
            mailNode.createRelationshipTo(userNode, RelationshipType.withName(MailListKnowledgeExtractor.MAIL_SENDER));
        else
            mailNode.createRelationshipTo(userNode, RelationshipType.withName(MailListKnowledgeExtractor.MAIL_RECEIVER));
    ...
    public static void createMailNode(Node node, String subject, String id, String senderName, String senderMail,
        ...
        node.addLabel(Label.label(MailListKnowledgeExtractor.MAIL));
        node.setProperty(MailListKnowledgeExtractor.MAIL_SUBJECT, subject);
        node.setProperty(MailListKnowledgeExtractor.MAIL_ID, id);
        node.setProperty(MailListKnowledgeExtractor.MAIL_SENDER_NAME, senderName);
        node.setProperty(MailListKnowledgeExtractor.MAIL_SENDER_MAIL, senderMail);
        node.setProperty(MailListKnowledgeExtractor.MAIL_RECEIVER_NAMES, String.join(", ", receiverNames));
        node.setProperty(MailListKnowledgeExtractor.MAIL_RECEIVER_MAILS, String.join(", ", receiverMails));
        node.setProperty(MailListKnowledgeExtractor.MAIL_DATE, date);
        node.setProperty(MailListKnowledgeExtractor.MAIL_BODY, body);
    }
