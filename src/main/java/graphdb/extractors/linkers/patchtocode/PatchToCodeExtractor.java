package graphdb.extractors.linkers.patchtocode;
 ...
 */
public class PatchToCodeExtractor implements Extractor {
        ...
        try (Transaction tx = db.beginTx()){
            for (Node node : db.getAllNodes()) {
                ...
                if (node.hasLabel(Label.label(JiraExtractor.PATCH))) {
                    String content = (String) node.getProperty(JiraExtractor.PATCH_CONTENT);
                    if (content != null) {
                        patchMap.put(content, node);
                    }
                } else if (node.hasLabel(Label.label(JavaCodeExtractor.CLASS))) {
