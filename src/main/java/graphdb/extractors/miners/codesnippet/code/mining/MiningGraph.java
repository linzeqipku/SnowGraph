...
import graphdb.extractors.miners.codesnippet.adt.LispList;
import graphdb.extractors.miners.codesnippet.adt.graph.impl.EdgeImpl;
...
//		});
	...
	public static <N, E> List<Graph<N, E>> resultFilter(Collection<Fragment<N, E>> result) {
		...
		List<Set<graphdb.extractors.miners.codesnippet.adt.graph.Edge<N>>> edgeSets = new ArrayList<>();
		result.stream().map(Fragment::toGraph).sorted(Ordering.natural().reverse().onResultOf(Graph::getEdgeCount)).forEach(graph -> {
			...
			Set<graphdb.extractors.miners.codesnippet.adt.graph.Edge<N>> edges = StreamSupport.stream(ite.spliterator(), false).map(EdgeImpl<N>::new).collect(Collectors.toSet());
			boolean add = true;
			for (Set<graphdb.extractors.miners.codesnippet.adt.graph.Edge<N>> edgeSet : edgeSets) {
