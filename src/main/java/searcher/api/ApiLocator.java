...
public class ApiLocator {
    ...
    private List<SubGraph> findSubGraphs(String queryString) {
        ...
        Set<String> queryWordSet = context.converter.convert(queryString);

        Set<String> tmpSet = new HashSet<>();
        for (String word : queryWordSet) { // 除去不在代码中出现过的词，可能会去掉同义词！
            if (context.word2Ids.containsKey(word) && context.word2Ids.get(word).size() > 0)
                tmpSet.add(word);
        }
        queryWordSet.clear();
        queryWordSet.addAll(tmpSet);

        scoreMap = ScoreUtils.getAPISimScore(queryWordSet, context.id2Words);
        ...
        if (anchors.size() > 0) {
            ...
            for (String queryWord : queryWordSet) {
                ...
                for (long id : subGraph)
                    if (context.id2Words.get(id).contains(queryWord)) {
                        hit = true;
                        break;
                    }
                ...
                long minDistNode = -1;
                for (long node : context.word2Ids.get(queryWord)) {
            ...
            for (long cNode : candidateNodes) {
                ...
                for (String queryWord : queryWordSet) {
                    if (context.word2Ids.get(queryWord).contains(cNode)) // 如果这个词包含在起始结点中，则跳过
                        continue;
                    double minDist = Double.MAX_VALUE;
                    long minDistNode = -1;
                    for (long node : context.word2Ids.get(queryWord)) { // 找到距离起始节点最近的加入进来
    ...
    private List<SubGraph> myFindSubGraphs(String queryString){
        ...
        Set<String> queryWordSet = context.converter.convertWithoutStem(queryString);
        // 计算 API score 和定位 anchor 时不stem 更加准确
        scoreMap = ScoreUtils.getAPISimScore(queryWordSet, context.id2OriginalWords);
        Set<Long> anchors = findAnchors(queryWordSet);

        // 做 beamsearch 时寻找候选时可以stem，扩大匹配的范围
        //queryWordSet = converter.convert(queryString);
        System.out.println(queryWordSet);
        ScoreUtils.generateCandidateMap(candidateMap, queryWordSet, context.word2Ids);
    ...
    private List<SubGraph> beamSearch(Set<String> querWordSet, List<SubGraph> initialGraph){
    ...
    private Set<Long> candidate(Set<String> queryWordSet) {
        ...
        for (long node : context.id2Name.keySet()) {
            int count = 0;
            for (String word : context.id2Words.get(node))
        ...
        for (String queryWord : queryWordSet)
            candidateNodes.addAll(context.word2Ids.get(queryWord));
