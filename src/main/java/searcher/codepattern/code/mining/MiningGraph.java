package searcher.codepattern.code.mining;

import searcher.codepattern.adt.LispList;
import searcher.codepattern.adt.graph.impl.EdgeImpl;
import searcher.codepattern.code.cfg.CFG;
import searcher.codepattern.code.cfg.ddg.DDG;
import searcher.codepattern.code.cfg.ddg.DDGBlock;
import searcher.codepattern.code.cfg.plaincfg.PlainCFG;
...
//		 添加常数、extern结点
	...
	public static <N, E> List<Graph<N, E>> resultFilter(Collection<Fragment<N, E>> result) {
		...
		List<Set<searcher.codepattern.adt.graph.Edge<N>>> edgeSets = new ArrayList<>();
		result.stream().map(Fragment::toGraph).sorted(Ordering.natural().reverse().onResultOf(Graph::getEdgeCount)).forEach(graph -> {
			...
			Set<searcher.codepattern.adt.graph.Edge<N>> edges = StreamSupport.stream(ite.spliterator(), false).map(EdgeImpl<N>::new).collect(Collectors.toSet());
			boolean add = true;
			for (Set<searcher.codepattern.adt.graph.Edge<N>> edgeSet : edgeSets) {
