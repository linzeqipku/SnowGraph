	...
	for (var i = 0; i < list.length; i++) {
		if (i === 0) {
			...
			if (list[i].type == list[i - 1].type) {
				...
				edge.push({
					nums: 1,
					begin: edge[edge.length - 1].end + edge[edge.length - 1].nums,
					end: edge[edge.length - 1].end + edge[edge.length - 1].nums,
					cata: list[i].type
				});
			}
		}
...
function init(json, node_ID) {
	...
	initGraph(json.searchResult, node_ID);
}
...
function initGraph(orijson, node_ID) {
	dataset = [];
	relationset = [];
	edgelist = [];
...
function sendcypher(data) {
	$.post("http://localhost:8080/CypherQuery", data, (result) => init(result, 10000));
}
